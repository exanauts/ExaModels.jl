...
function distillation_column_model(T = 3, backend = nothing)
    ...
    xA = ExaModels.variable(c, 0:T, 0:NT+1; start = 0.5)
    yA = ExaModels.variable(c, 0:T, 0:NT+1; start = 0.5)
    u = ExaModels.variable(c, 0:T; start = 1.0)
    V = ExaModels.variable(c, 0:T; start = 1.0)
    L2 = ExaModels.variable(c, 0:T; start = 1.0)
    ...
    ExaModels.constraint(c, V[t] - u[t] * D - D for t = 0:T)
    ExaModels.constraint(c, L2[t] - u[t] * D - F for t = 0:T)
    ExaModels.constraint(
        c,
...
````
distillation_column_model (generic function with 3 methods)
````
