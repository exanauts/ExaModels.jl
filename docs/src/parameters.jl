...
N = 10
x_p = variable(c_param, N; start = (mod(i, 2) == 1 ? -1.2 : 1.0 for i in 1:N))

# Now we can use the parameters in our objective function just like variables:
objective(c_param, θ[1] * (x_p[i - 1]^2 - x_p[i])^2 + (x_p[i - 1] - θ[2])^2 for i in 2:N)
...
constraint(
    ...
    3x_p[i + 1]^3 + 2 * x_p[i + 2] - 5 +
        ...
        4x_p[i + 1] - x_p[i]exp(x_p[i] - x_p[i + 1]) - 3 for i in 1:(N - 2)
)
