...
#
# This example demonstrates the use of [`@expr`](@ref) to define reusable subexpressions
...
#
# The [`@expr`](@ref) macro (or [`add_expr`](@ref)) creates an **inlined subexpression**:
...
#
# ```julia
# @expr(c, s, x[i]^2 for i in 1:n)   # s[i] expands to x[i]^2 at each use site
# ```
#
