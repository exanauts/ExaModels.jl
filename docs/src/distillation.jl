...
# ```julia
# @expr(c, s, x[i]^2 for i in 1:n)  # adds n variables + n constraints
# ```
#
# **Reduced subexpressions**: Inlines the expression directly, no extra variables or constraints.
# ```julia
# @expr(c, s, x[i]^2 for i in 1:n; reduced=true)  # no extra vars/cons
# ```
#
...
function distillation_column_model_with_subexpr(T = 3; backend = nothing)
    ...
    @expr(c, dxA, (xA[t, i] - xA[t - 1, i]) / dt for t in 1:T, i in 0:(NT + 1))
    @expr(c, dyA, yA[t, i] - yA[t, i + 1] for t in 0:T, i in 0:NT)
    ...
    @con(c, dxA[t, 0] - (1 / Ac) * (yA[t, 1] - xA[t, 0]) for t in 1:T)

    ## Rectifying section - cleaner with dxA and dyA
    itr_rect = ExaModels.convert_array(collect(Iterators.product(1:T, 1:(FT - 1))), backend)
    @con(c, dxA[t, i] - (1 / At) * (u[t] * D * (yA[t, i - 1] - xA[t, i]) - V[t] * dyA[t, i]) for (t, i) in itr_rect)

    ## Feed tray
    @con(c, dxA[t, FT] - (1 / At) * (F * xAf + u[t] * D * xA[t, FT - 1] - L2[t] * xA[t, FT] - V[t] * dyA[t, FT]) for t in 1:T)

    ## Stripping section
    itr_strip = ExaModels.convert_array(collect(Iterators.product(1:T, (FT + 1):NT)), backend)
    @con(c, dxA[t, i] - (1 / At) * (L2[t] * (yA[t, i - 1] - xA[t, i]) - V[t] * dyA[t, i]) for (t, i) in itr_strip)

    ## Reboiler
    @con(c, dxA[t, NT + 1] - (1 / Ar) * (L2[t] * xA[t, NT] - (F - D) * xA[t, NT + 1] - V[t] * yA[t, NT + 1]) for t in 1:T)
    ...
    @con(c, yA[t, i] * (1 - xA[t, i]) - alpha * xA[t, i] * (1 - yA[t, i]) for (t, i) in itr_vle)
...
function distillation_column_model_reduced(T = 3; backend = nothing)
    ...
    @expr(c, dxA, (xA[t, i] - xA[t - 1, i]) / dt for t in 1:T, i in 0:(NT + 1); reduced = true)
    @expr(c, dyA, yA[t, i] - yA[t, i + 1] for t in 0:T, i in 0:NT; reduced = true)
