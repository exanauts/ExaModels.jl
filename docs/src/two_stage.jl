...
# To annotate the scenario for each variable and constraint, we can use the `scenario` keyword. We need to start with a special ExaCore that supports such scenario annotations, which can be created by calling `TwoStageExaCore(; ns = ...)`.
core = TwoStageExaCore(; ns = 3)

# Now we can define the design variable and recourse variables. The `scenario` keyword argument allows us to specify which scenario(s) each variable belongs to. For the design variable `d`, we set `scenario = false` to indicate that it is shared across all scenarios (a design variable).
d = variable(core; start = 1.0, lvar = 0.0, uvar = Inf, scenario = false)  ## design variable d

# For the recourse variables `v`, we specify `scenario = true` to indicate that these are recourse (scenario-specific) variables. This allows us to define scenario-specific constraints and objectives that involve these recourse variables.
v = variable(core, ns, nv; start = 1.0, lvar = 0.0, uvar = Inf, scenario = true)  ## recourse variables v
...
constraint(core, v[s, 1] - v[s, 2]^2 for s in 1:ns; lcon = 0.0, scenario = true)
