...
\begin{enumerate}
  ...
  \item[\textbf{I1.}] A full implementation of the above ideas in ExaModels.jl with cross-platform GPU portability across NVIDIA, AMD, Intel, and Apple devices via KernelAbstractions.jl \cite{churavyKernelAbstractionsjl2025}---the same model code runs on all backends without modification.
...
\end{enumerate}
We demonstrate substantial speedups in sparse derivative evaluation over existing AMSs on CPU and up to two orders of magnitude further acceleration on GPUs, validated on benchmarks from the Luk\v{s}an--Vlc\v{e}k \cite{lukVsanSparsePartiallySeparable}, COPS \cite{dolanBenchmarkingOptimizationSoftware2001}, and PGLIB-OPF suites.
...
Table~\ref{tab:hardware} summarizes the hardware used in the experiments. The AD comparison (Section~\ref{sec:numerics:ad}) runs single-threaded on the CPU of Machine~A. The GPU experiments (Section~\ref{sec:numerics:gpu}) compare this CPU baseline against two GPU backends on the same machines. All experiments use Julia~1.12.6, ExaModels.jl~v0.10.0, and Float64 arithmetic.
