...
\maketitle
\begin{abstract}
  Large-scale nonlinear programs almost always exhibit highly repetitive structure, yet existing algebraic modeling systems often neglect it, flattening it into monolithic expression graphs and forgoing the parallelism it affords. We present ExaModels.jl, an algebraic modeling system for nonlinear programming implemented in Julia that preserves and exploits this structure for scalable, GPU-compatible derivative evaluation. At its core is a \emph{single-instruction, multiple-data (SIMD) abstraction} that represents a nonlinear program as a small number of computational patterns, each evaluated over many data points; problem structure is recognized at compile time, so function and derivative evaluation reduces to embarrassingly parallel loops over data points, making the system \emph{natively compatible with GPUs}. The sparse automatic differentiation built on this abstraction further exploits the regular per-pattern sparsity to compute Jacobians and Hessians in a partially compressed sparse form without graph coloring. These ideas deliver substantial speedups in sparse first- and second-order derivative evaluation over existing algebraic modeling systems on CPU and up to two orders of magnitude further acceleration on GPUs. Additionally, our implementation supports arbitrary numeric precision, cross-platform GPU portability across NVIDIA, AMD, Intel, and Apple devices, ahead-of-time compilation to standalone binaries that do not depend on the Julia runtime (currently supported on CPU), as well as extensions to parametric problems, problems with user-defined external oracles, two-stage stochastic programs, and batch optimization. We demonstrate the approach with ExaModels-native implementations of the Luk\v{s}an--Vlc\v{e}k, COPS, and PGLIB-OPF benchmark libraries, comparing AD performance against JuMP and AMPL on CPU and across multiple GPU backends.
\end{abstract}
