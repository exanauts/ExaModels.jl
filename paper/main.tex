...
core = ExaCore(minimize = false)        % maximize $h_T$
...
@add_con(core, c1,                     % altitude dynamics \eqref{eq:rocket:h}
  -h[i]+h[i-1] + 0.5*step[1]*(v[i]+v[i-1]) for i=1:nh)
@add_con(core, c2,                     % velocity dynamics \eqref{eq:rocket:v}
...
Figure~\ref{fig:ad} illustrates both passes on the expression $f(x) = \sin(x_1) + x_1 \cdot x_2$.

\begin{figure}[t]
\centering
\begin{tikzpicture}[
  scale=0.82, transform shape,
  op/.style={circle, draw, thick, minimum size=9mm, inner sep=1pt, font=\small\ttfamily},
  var/.style={rectangle, rounded corners=2pt, draw, thick, fill=j1!20, minimum size=7mm, inner sep=2pt, font=\small\ttfamily},
  fwdlabel/.style={font=\scriptsize, text=ForestGreen, anchor=west},
  revlabel/.style={font=\scriptsize, text=Maroon, anchor=east},
...
% Forward pass (left)
\node[font=\small\bfseries, anchor=south] at (0,1.2) {Forward pass};
\node[op] (plus) {$+$}
  child { node[op] (sin) {sin}
    child { node[var] (x1a) {$x_1$} }
  }
  child { node[op] (times) {$\times$}
    child { node[var] (x1b) {$x_1$} }
    child { node[var] (x2) {$x_2$} }
  };

% Forward annotations (primal value, local derivative)
\node[fwdlabel, right=2pt of plus] {$v{=}\sin x_1 {+} x_1 x_2$};
\node[fwdlabel, right=2pt of sin] {$v{=}\sin x_1,\; F'{=}\cos x_1$};
\node[fwdlabel, right=2pt of times] {$v{=}x_1 x_2,\; \partial_1{=}x_2,\; \partial_2{=}x_1$};

% Reverse pass (right)
\begin{scope}[xshift=85mm]
\node[font=\small\bfseries, anchor=south] at (0,1.2) {Reverse pass};
\node[op] (rplus) {$+$}
  child { node[op] (rsin) {sin}
    child { node[var] (rx1a) {$x_1$} }
  }
  child { node[op] (rtimes) {$\times$}
    child { node[var] (rx1b) {$x_1$} }
    child { node[var] (rx2) {$x_2$} }
  };

% Reverse annotations (adjoint values)
\node[revlabel, left=2pt of rplus] {$\bar{v}{=}1$};
\node[revlabel, left=2pt of rsin] {$\bar{v}{=}1$};
\node[revlabel, left=2pt of rtimes] {$\bar{v}{=}1$};
\node[revlabel, left=10pt of rx1a] {$\bar{v}{=}\cos x_1$};
\node[revlabel, left=2pt of rx1b] {$\bar{v}{=}x_2$};
\node[font=\scriptsize, text=Maroon, anchor=west, right=2pt of rx2] {$\bar{v}{=}x_1$};

% Arrows showing adjoint flow
\draw[->, Maroon, thick, dashed] (rplus) -- node[left, font=\scriptsize\color{Maroon}, pos=0.4] {$1{\cdot}1$} (rsin);
\draw[->, Maroon, thick, dashed] (rplus) -- node[right, font=\scriptsize\color{Maroon}, pos=0.4] {$1{\cdot}1$} (rtimes);
\draw[->, Maroon, thick, dashed] (rsin) -- node[left, font=\scriptsize\color{Maroon}, pos=0.4] {$1{\cdot}\cos x_1$} (rx1a);
\draw[->, Maroon, thick, dashed] (rtimes) -- node[left, font=\scriptsize\color{Maroon}, pos=0.4] {$1{\cdot}x_2$} (rx1b);
\draw[->, Maroon, thick, dashed] (rtimes) -- node[right, font=\scriptsize\color{Maroon}, pos=0.4] {$1{\cdot}x_1$} (rx2);
\end{scope}
\end{tikzpicture}
\caption{First-order AD for $f(x) = \sin(x_1) + x_1 x_2$. \textbf{Left:} The forward pass evaluates the adjoint tree, storing the primal value $v$ and local derivatives $F'$, $\partial_1$, $\partial_2$ at each node.  \textbf{Right:} The reverse pass propagates the adjoint $\bar{v}$ from root to leaves; at each \texttt{AdjointNodeVar} leaf, the accumulated adjoint gives the gradient entry: $\nabla f = (\cos x_1 + x_2,\; x_1)^\top$.}
\label{fig:ad}
\end{figure}
...
We compare ExaModels.jl on CPU (single-threaded, no KernelAbstractions) against GPU backends (CUDA, ROCm) on the same problem instances. The GPU timings include kernel execution time but exclude the one-time model construction (which runs on the CPU). We report speedup relative to the CPU baseline for each callback.

% TODO: insert GPU speedup figure here once benchmark results are collected
