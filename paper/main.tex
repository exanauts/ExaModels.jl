...
\maketitle
\begin{abstract}
  Large-scale nonlinear programs almost always exhibit highly repetitive structure, yet existing algebraic modeling systems often neglect it, forgoing the parallelism it affords. We present ExaModels.jl, an algebraic modeling system for nonlinear programming implemented in Julia that preserves and exploits this structure for scalable, GPU-compatible derivative evaluation. At its core is a \emph{single-instruction, multiple-data abstraction} that represents a nonlinear program as a small number of algebraic patterns, each evaluated over many data points; because problem structure is recognized at compile time, function and derivative evaluation reduces to embarrassingly parallel loops, making the system \emph{natively compatible with GPUs}. The sparse derivative evaluation built on this abstraction further exploits regular per-pattern sparsity to compute Jacobians and Lagrangian Hessians in partially compressed sparse form without graph coloring. These ideas deliver substantial speedups in sparse derivative evaluation over existing algebraic modeling systems on CPU and up to two orders of magnitude further acceleration on GPUs. Additionally, our implementation supports arbitrary numeric precision, cross-platform GPU portability across NVIDIA, AMD, Intel, and Apple devices, ahead-of-time compilation to standalone binaries independent of the Julia runtime (currently on CPU), and extensions to parametric problems, user-defined external oracles, two-stage stochastic programs, and batch optimization. We demonstrate the approach with ExaModels-native implementations of the Luk\v{s}an--Vlc\v{e}k, COPS, and PGLIB-OPF benchmark libraries, comparing derivative evaluation performance against JuMP and AMPL on CPU and across multiple GPU backends.
\end{abstract}
...
Large-scale nonlinear programs (NLPs) arising in engineering, statistics, and scientific applications almost always exhibit highly repetitive structure: the same algebraic pattern appears across many data points in both the objective and constraints.
For instance, the alternating current optimal power flow (ACOPF) problem---one of the most widely studied NLPs in power systems---can be expressed with only 15 distinct algebraic patterns, regardless of whether the network contains hundreds or tens of thousands of buses \citep{shinAcceleratingOptimalPower2024}. Exploiting this repetitive structure is key to efficiently evaluating model functions and their derivatives, which in turn is crucial for the overall performance of the nonlinear optimization solution procedure. This paper presents a modeling abstraction, an automatic differentiation (AD) algorithm, and their software implementation in ExaModels.jl that exploit this structure for scalable derivative evaluation and GPU acceleration for large-scale NLPs.

To make this concrete, consider the Goddard rocket problem \citep{goddardMethodReachingExtreme1919} from the COPS benchmark suite \citep{dolanBenchmarkingOptimizationSoftware2001}, discretized via the trapezoidal rule with $T$ time steps:
\begin{subequations}\label{eq:rocket}
\begin{align}
  \max_{\substack{h_t,\, v_t,\, m_t,\, \tau_t \\ \Delta t}} \quad
  & h_T \\
  \st \quad
...
\end{align}
\end{subequations}
where $h_t$, $v_t$, $m_t$, and $\tau_t$ are the altitude, velocity, mass, and thrust at time $t$; $D(h,v) = D_c v^2 e^{-h_c(h - h_0)/h_0}$ is the aerodynamic drag; $g(h) = g_0 (h_0/h)^2$ is the gravitational acceleration; and $c$ is the exhaust velocity. The problem aims to maximize the final altitude of a vertically ascending rocket subject to aerodynamic drag and gravity. This is a classical optimal control problem representative of the discretize-then-optimize approach widely used in trajectory optimization.

The function and derivative evaluations of this problem are naturally iterator-representable. Consider the altitude dynamics \cref{eq:rocket:h}: the algebraic expression $h_t - h_{t-1} - \tfrac{\Delta t}{2}(v_t + v_{t-1}) = 0$ has the same functional form for every $t \in [T]$---only the variable indices change. The same holds for the velocity dynamics \cref{eq:rocket:v} and the mass dynamics \cref{eq:rocket:m}. Each constraint type defines a single algebraic pattern, and the time indices $t = 1, \ldots, T$ are the data points over which that pattern is evaluated. Evaluating the constraint function or its derivatives therefore reduces to a \texttt{for} loop over $t$ for each pattern, and these loops are embarrassingly parallel across data points. A consequence is that when the discretization is refined, the problem size grows proportionally in $T$, but the number of distinct algebraic patterns---the three dynamics constraints \cref{eq:rocket:h}--\cref{eq:rocket:m} and the boundary conditions---remains fixed at five.

This iterator-representable structure is not peculiar to the Goddard rocket problem in \cref{eq:rocket}. Large-scale NLPs arising in engineering, statistics, and scientific applications---optimal power flow, PDE-constrained optimization, model predictive control, network design, maximum likelihood estimation---routinely involve millions of variables and constraints, yet their mathematical formulations fit on at most a few pages precisely because the same algebraic expressions are repeated over spatial nodes, time steps, scenarios, or data samples. The problem size is driven by the number of data points, not the number of distinct algebraic patterns. 

Thus, for a modeling system to effectively and concisely express this structure, it should allow users to write their model in a way that preserves the pattern--data separation. When this separation is maintained, each algebraic pattern needs to be analyzed and differentiated only once; the resulting kernels can then be applied across all data points in parallel if needed, opening the door to GPU acceleration and other structure-exploiting optimizations in derivative evaluation. This observation motivates the following question:
\begin{quote}
...
An algebraic modeling system (AMS) sits between the user's mathematical formulation and the numerical solver. The user writes the problem in a high-level syntax. The AMS produces the data the solver consumes: matrices and vectors for linear and quadratic problems, and callbacks---objective, gradient, constraints, Jacobian, Lagrangian Hessian---for nonlinear problems. Automatic differentiation is relevant only to the nonlinear case, and many AMSs delegate it to external tooling rather than implementing their own. ExaModels.jl is an AMS that ships its own AD; it is not a solver, and provides only the modeling interface and callback functions. NLP solvers---e.g., Ipopt \citep{wachterImplementationInteriorpointFilter2006}, KNITRO \citep{byrdKnitroIntegratedPackage2006}, and MadNLP.jl \citep{shinMadNLPjl2025}---are not reviewed here. The comparison below is restricted to AMSs that target general smooth NLPs of the form \cref{eq:nlp} and ship with their own AD. Pyomo and CVXPY do not satisfy these criteria and are discussed at the end of the section.

\paragraph{Algebraic modeling systems with own AD.}
\Cref{tab:ams} summarizes the six AMSs we consider. AMPL \citep{fourerModelingLanguageMathematical1990} is a standalone language with its own parser. Function and derivative callbacks are exposed through the AMPL Solver Library (ASL) via the \texttt{.nl} interchange format. GAMS \citep{bussieckGeneralAlgebraicModeling2004} is contemporaneous and similar in design. JuMP \citep{dunningJuMPModelingLanguage2017,lubinJuMP10Recent2023} is embedded in Julia. It uses syntactic macros---rather than operator overloading---to build a per-scalar expression graph, and runs reverse-mode AD on the graph. CasADi \citep{anderssonCasADiSoftwareFramework2019} is a C++ symbolic framework. Its central abstraction is the \texttt{Function}: a compiled expression graph that can be reused, embedded, or emitted as standalone C code. CasADi supports forward and reverse AD with graph-coloring sparsity detection. GEKKO \citep{bealGEKKOOptimizationSuite2018} is a Python frontend; its APMonitor backend handles compilation, AD, and dynamic discretization. Gravity \citep{hijaziGravityMathematicalModeling2018} introduces ``constraint templates'' to preserve the pattern structure of indexed constraints, and reports order-of-magnitude function-evaluation and memory improvements over JuMP on network optimization.

\begin{table}[t]
  \centering
  \caption{Algebraic modeling systems with their own AD that target smooth NLPs. ``Pattern-aware'' indicates whether the system preserves the repetitive structure of indexed constraints for SIMD-parallel derivative evaluation.}
  \label{tab:ams}
  \small
  ...
  \begin{tabular}{l l l l c}
    ...
    \midrule
    AMPL         & own           & LP, MILP, NLP        & ASL (built-in)            & no \\
    GAMS         & own           & LP, MILP, NLP        & built-in                  & no \\
    JuMP         & Julia         & LP, MILP, conic, NLP & reverse-mode on graph     & no \\
    CasADi       & C++/Py/MATLAB & NLP, OCP             & source transform          & partial \\
    GEKKO        & Python        & NLP, dynamic         & APMonitor backend         & no \\
    Gravity      & C++           & NLP (network)        & symbolic                  & yes \\
    \midrule
    ExaModels.jl & Julia         & NLP                  & type-stable per-pattern   & yes \\
    \bottomrule
  \end{tabular}
\end{table}

AMPL, GAMS, and JuMP construct flat, per-scalar expression graphs at model construction. Every constraint instance becomes a node in a single large graph. AD traverses this graph at evaluation time. JuMP is the canonical example of iterator flattening: indexed-constraint syntax such as \texttt{@constraint(model, [i=1:N], ...)} is lowered to a flat scalar collection before AD, so the iteration visible to the user is no longer visible to the AD machinery. 

For users who already have JuMP models, ExaModels.jl provides a translation layer (\texttt{ExaModelsMOI.jl}) that recovers patterns from an MOI model by bucketing scalar constraints with matching expression heads into templatized sums. This is structurally limited: pattern recognition runs after JuMP has already enumerated each constraint, and structure that JuMP collapsed at submission cannot be reconstructed. \texttt{GenOpt.jl} \citep{legatGenOptJl2025} is an actively developed JuMP extension that adds constraint and sum generators, preserving iteration structure at JuMP-side construction so ExaModels receives the patterns intact.

CasADi compiles each \texttt{Function} once and reuses it across calls; its symbolic types are scalar (SX) and matrix (MX). User-side iteration written as a Python or MATLAB loop is enumerated into the symbolic graph regardless of which type. Vectorized MX expressions are the exception: the slice \texttt{x[1:] - x[:-1]} stays a single matrix-level node, and both the symbolic graph and the generated C function body remain constant-size as the discretization grows---only the loop bound and workspace offsets change. SX and Python-loop MX both expand fully: a 100-step problem produces hundreds of lines of unrolled C, and a 10{,}000-step problem produces tens of thousands. Vectorized MX, however, requires the model to be written as matrix algebra. Heterogeneous indexed structure---variable-size sums per index, as in ACOPF power balance over each bus's generators and incident lines---can be encoded via sparse incidence matrices and sparse matrix-vector products. The result is mathematically equivalent to a templatized model, but uses a different abstraction: matrix algebra over pre-constructed sparsity patterns versus per-index expressions over data sets.

Two strategies preserve indexed-constraint structure at the modeling layer: vectorization and templatizing. Vectorization expresses an indexed family as a single matrix-algebra operation; CasADi's vectorized MX is the canonical example. Templatizing parameterizes a single expression over a data set and evaluates it across instances; Gravity's constraint templates and ExaModels.jl's (pattern, data iterator) pair are the canonical examples. The two strategies differ in abstraction rather than reach: heterogeneous indexed structure---variable-size sums per index---can be expressed in either form, but vectorization shifts the work to pre-constructing sparse incidence matrices while templatizing exposes per-index sums directly. ExaModels.jl falls in the templatizing family and is closest in spirit to Gravity, with two refinements: type-stable parameterized expression trees compiled per pattern via the Julia compiler (\Cref{sec:graph}), and GPU-native derivative evaluation through KernelAbstractions.jl (\Cref{sec:gpu}). Neither CasADi nor Gravity supports GPU-native derivative evaluation. CusADi \citep{jeonCusADiGPUParallelization2025} is an external CasADi extension that targets GPU parallelism across many independent problem instances, a setting common in reinforcement-learning policy training and parameter sweeps. The within-instance pattern parallelism considered here is a complementary axis.

A second axis is the evaluation path: compiled vs interpreted. Compilation reduces per-callback runtime overhead at the cost of longer build time. AMPL parses models to the \texttt{.nl} format, which is evaluated by ASL with compiled operator implementations; a legacy \texttt{nlc} tool can emit standalone C. GAMS uses a similar built-in pipeline. CasADi provides both options: by default a \texttt{Function} is evaluated through CasADi's runtime, which walks a flat operation sequence with compiled operator implementations; alternatively, standalone C code can be emitted via codegen. GEKKO compiles to bytecode through APMonitor; the bytecode is interpreted at evaluation time. Gravity uses symbolic differentiation and dispatches compiled C++ templates. JuMP is embedded in Julia. Its default AD walks the symbolic expression graph at every callback and is interpreted in practice; an alternative backend, SymbolicAD.jl, generates compiled callbacks via symbolic differentiation. ExaModels.jl is type-stable end-to-end, and the Julia compiler specializes a dedicated evaluator for each pattern. Compilation is also necessary for GPU execution, where dynamic dispatch is not supported (\Cref{sec:aot}).

\paragraph{Out of scope: Pyomo and CVXPY.}
Pyomo \citep{hartPyomoModelingSolving2011,hartPyomoOptimizationModeling2017} supports general NLP but does not implement its own AD. Models are serialized to \texttt{.nl} and derivatives are obtained from ASL. A direct benchmark would measure ASL, not the modeling layer, and Pyomo would duplicate AMPL in \Cref{tab:ams}. CVXPY \citep{diamondCVXPYPythonEmbeddedModeling2016} canonicalizes convex problems into cone form and dispatches them to a cone solver. It targets a different problem class and a different solution route from \cref{eq:nlp}, and has no derivative-callback API. The same scope criteria exclude DCP-based systems---CVX \citep{grantCVXMATLABSoftware2014}, Convex.jl \citep{udellConvexOptimizationJulia2014}, and YALMIP \citep{lofbergYALMIPToolboxModeling2004}---and AD-delegating bridges such as \texttt{AmplNLWriter.jl}.

\paragraph{Machine-learning AD frameworks.}
PyTorch \citep{paszkePyTorchImperativeStyle2019}, JAX \citep{bradburyJAXComposableTransformations2018}, TensorFlow \citep{abadiTensorFlowLargescaleMachine2016}, Zygote.jl \citep{innesDontUnrollAdjoint2018}, and Enzyme \citep{mosesInsteadRewritingForeign2020} provide AD and GPU support. They could in principle express a smooth NLP as a differentiable function. They are not modeling systems: declaring constraints, objectives, and variable bounds is left to the user. Their AD is tuned for dense gradients with batch dimensions; sparse second-order derivatives in $(r, c, v)$ form are limited (\texttt{jax.experimental.sparse}) or absent. Their parallelism primitives (\texttt{vmap}, \texttt{pmap}) act at the array level, not the algebraic-pattern level. ExaModels.jl, like the AMSs in \Cref{tab:ams}, sits at a higher level of abstraction.

\paragraph{Domain-specific extensions.}
AMSs are commonly extended for particular problem classes. JuMP extensions include Plasmo.jl \citep{jalvingGraphbasedModelingAbstraction2022}, InfiniteOpt.jl \citep{pulsipherUnifyingModelingAbstraction2022}, PowerModels.jl \citep{coffrinPowerModelsJLOpenSource2018}, and SDDP.jl \citep{dowsonSDDPjlJuliaPackage2021}. Pyomo extensions include IDAES \citep{leeIDAESProcessModeling2021}, pyomo.dae \citep{nicholsonPyomoDAEModeling2018}, and OMLT \citep{cecconOMLTOptimizationMachine2022}. ExaModelsPower.jl \citep{shinExaModelsPowerjl2024} provides AC and DC OPF formulations and serves as a benchmark library in \Cref{sec:numerics}.

\subsection{Our approach}
\label{sec:intro:approach}
ExaModels.jl adopts a philosophy that is slightly more restrictive than general-purpose AMSs: it \emph{requires} the user to express the model in a structured form where each objective term and constraint is specified as a algebraic pattern paired with a data iterator. This is a deliberate design choice---by asking the user to expose the parallelizable structure of their problem, ExaModels.jl can exploit it for efficient derivative evaluation. The resulting NLP model information is stored as a collection of patterns and their associated data points. For each pattern, a single derivative evaluation kernel is compiled once and executed across all data points in a SIMD fashion. This is the key to enabling GPU acceleration for large-scale NLPs.

Another key contribution of ExaModels.jl is the \emph{parameterized expression tree}, the core data structure for representing NLP model information. Each expression tree encodes a algebraic pattern as a type-stable graph of operations, parameterized by problem data supplied at evaluation time. This design allows the expression tree to be evaluated and differentiated with different data and arguments without recompilation---a property that is crucial for GPU execution, where we compile the derivative evaluation kernels once and execute them across many data points.

Furthermore, we implement a novel sparse reverse-mode AD algorithm that does not require graph coloring. This is enabled by an \emph{a priori} sparsity analysis in which the relative index sequence of nonzero entries within each pattern is analyzed at model-construction time and compressed into a compact mapping. This allows us to directly compute the sparse Jacobian and Lagrangian Hessian in coordinate format without the overhead of coloring, which is especially beneficial for GPU execution where dynamic control flow can be costly. 

ExaModels.jl provides a convenient user syntax for model construction, including macros for adding variables, parameters, objectives, and constraints, as well as for registering custom functions with their first and second derivatives. Multi-backend GPU execution is supported via KernelAbstractions.jl \citep{churavyKernelAbstractionsjl2025}, allowing users to run on NVIDIA (CUDA), AMD (ROCm), Intel (oneAPI), or Apple (Metal) accelerators without modifying their model code. We demonstrate the performance of ExaModels.jl on a range of benchmark problems and quantify the speedup over existing AMSs and over CPU baselines.
...
ExaModels.jl is implemented in Julia \citep{bezansonJuliaFreshApproach2017}, a just-in-time (JIT) compiled language with a powerful type system and multiple dispatch. The user constructs the model in a high-level syntax, and the underlying data structures and AD algorithms are compiled and optimized for the specific model structure. We leverage Julia's multiple dispatch to enable efficient function registration and maintain type stability throughout the model construction and derivative evaluation process. Importantly, ExaModels.jl is designed to be compatible with ahead-of-time (AOT) compilation via \texttt{juliac} with trimming: it does not rely on runtime code generation or dynamic dispatch in the hot evaluation paths. This makes ExaModels.jl suitable for deployment in production environments where AOT compilation is required, placing it on equal footing with AMSs that feature dedicated code generation, such as CasADi \citep{anderssonCasADiSoftwareFramework2019}.

\paragraph{Relationship to prior work.}
The SIMD abstraction first appeared in \citet{shinAcceleratingOptimalPower2024} as part of an ACOPF-specific GPU optimization framework, but the abstraction itself, the parameterized expression tree, the coloring-free sparse AD algorithm, and the multi-backend GPU implementation have not previously been elaborated; this is the role of the present paper. Several subsequent works use ExaModels.jl as the modeling layer: \citet{shinExaModelsPowerjl2024} provides a power-systems library on top of it; the GPU-native condensed-space interior-point methods of \citet{pacaudCondensedspaceMethodsNonlinear2024} consume its callbacks; and \citet{pacaudGPUacceleratedNonlinearModel2024} reports a fully GPU-resident optimization pipeline pairing ExaModels.jl with MadNLP.jl~\citep{shinMadNLPjl2025}.

\subsection{Contributions}
\label{sec:intro:contributions}
The contributions of this paper are methodological (M), implementational (I), and experimental (E).
\begin{enumerate}
  \item[\textbf{M1.}] A \emph{single-instruction, multiple-data abstraction} for NLPs that represents a problem as a small number of algebraic patterns, each evaluated over many data points. This abstraction is realized through a parameterized expression tree, associated data structures, and a user-facing interface that preserve the pattern--data separation throughout model construction. Because problem structure is recognized at compile time, function and derivative evaluation reduces to embarrassingly parallel loops over data points, making the system natively compatible with GPUs.
  \item[\textbf{M2.}] A \emph{coloring-free sparse AD algorithm} that exploits regular per-pattern sparsity to compute Jacobians and Hessians in partially compressed sparse form without graph coloring.
  \item[\textbf{I1.}] A full implementation of the above ideas in ExaModels.jl with cross-platform GPU portability across NVIDIA, AMD, Intel, and Apple devices via KernelAbstractions.jl \citep{churavyKernelAbstractionsjl2025}---the same model code runs on all backends without modification.
  \item[\textbf{I2.}] Ahead-of-time compilation to standalone binaries independent of the Julia runtime, with no separate code generation step: the same code paths serve both just-in-time and ahead-of-time compilation.
  \item[\textbf{I3.}] Support for arbitrary numeric precision---the same model code runs in Float32, Float64, or user-defined types without modification.
  \item[\textbf{I4.}] Variants for parametric problems, user-defined external oracles, two-stage stochastic programs, and batch optimization.
  \item[\textbf{E1.}] ExaModels-native implementations of the Luk\v{s}an--Vlc\v{e}k \citep{lukVsanSparsePartiallySeparable}, COPS \citep{dolanBenchmarkingOptimizationSoftware2001}, and PGLIB-OPF benchmark libraries, with systematic comparison of derivative evaluation performance against JuMP and AMPL on CPU and across multiple GPU backends. We report speedups over existing AMSs on CPU and up to two orders of magnitude further acceleration on GPUs.
\end{enumerate}

\subsection{Outline}
\label{sec:intro:outline}
The remainder of this paper is organized as follows. \Cref{sec:modeling} introduces the modeling abstraction, the SIMD formulation, and the user-facing syntax. \Cref{sec:data} describes the core data structures. \Cref{sec:graph} presents the parameterized expression tree. \Cref{sec:ad} develops the sparse automatic differentiation algorithm. \Cref{sec:gpu} describes GPU acceleration via KernelAbstractions.jl. \Cref{sec:aot} addresses compile-time design. \Cref{sec:extensions} introduces parametric, two-stage, and batch variants. \Cref{sec:numerics} reports numerical experiments. \Cref{sec:conclusions} concludes the paper.

\subsection{Notation}
\label{sec:intro:notation}
We use $[N] := \{1,\ldots,N\}$ for index sets and $\mathbb{R}^n$ for $n$-dimensional real space. Decision variables are denoted $x \in \mathbb{R}^n$ with bounds $x^\flat \leq x \leq x^\sharp$; dual variables (Lagrange multipliers) are $\blambda \in \mathbb{R}^m$. The parameter vector is $\theta$. In \cref{eq:simd}, $f^{(\ell)}$, $g^{(m)}$, $h^{(n)}$ denote algebraic patterns with associated data collections $\mathcal{D}_\ell$, and $p$, $q$, $s$ denote data points within those collections. Gradients $\nabla f$ are column vectors; the Jacobian $\nabla g$ has dimensions $m \times n$ with rows $\nabla g_j^\top$. Code identifiers are typeset in \texttt{monospace}.
...
\end{equation}
where $f:\mathbb{R}^n \to \mathbb{R}$ is the objective function, $g:\mathbb{R}^n \to \mathbb{R}^m$ is the constraint function, and $x^\flat, x^\sharp, g^\flat, g^\sharp$ are the variable and constraint bounds. Interior-point methods \citep{wachterImplementationInteriorpointFilter2006,nocedalNumericalOptimization2006} and other NLP solvers require the following five callbacks from the modeling layer:
\begin{itemize}
  \item \texttt{NLPModels.obj}$(m, x) \to f(x) \in \mathbb{R}$ --- objective value;
  \item \texttt{NLPModels.grad!}$(m, x, \nabla\!f) \to \nabla f(x) \in \mathbb{R}^n$ --- objective gradient;
  \item \texttt{NLPModels.cons!}$(m, x, c) \to g(x) \in \mathbb{R}^m$ --- constraint values;
  \item \texttt{NLPModels.jac\_coord!}$(m, x, v) \to \nabla g(x) \in \mathbb{R}^{m \times n}$ --- sparse Jacobian;
  \item \texttt{NLPModels.hess\_coord!}$(m, x, \blambda, v) \to \nabla^2 f(x) + \sum_{j=1}^m \lambda_j \nabla^2 g_j(x) \in \mathbb{R}^{n \times n}$ --- sparse Lagrangian Hessian,
\end{itemize}
where $m$ is the model object. These callbacks follow the NLPModels.jl interface \citep{jsoNLPModelsJl} developed by the JuliaSmoothOptimizers (JSO) organization. NLPModels.jl defines the abstract type \texttt{AbstractNLPModel} and a standardized callback API that decouples the modeling layer from the solver layer: any model that implements this interface is accepted by any compatible solver. Compatible NLP solvers include Ipopt \citep{wachterImplementationInteriorpointFilter2006} (via NLPModelsIpopt.jl), KNITRO \citep{byrdKnitroIntegratedPackage2006} (via NLPModelsKnitro.jl), MadNLP.jl \citep{shinMadNLPjl2025}, Uno (via Uno.jl), Percival.jl, DCISolver.jl, and FletcherPenaltySolver.jl. The \texttt{!}-suffixed callbacks write their result into a preallocated output array. The Jacobian and Lagrangian Hessian are stored in coordinate (COO) format---triplets $(r, c, v)$ of row indices, column indices, and values---since the sparsity structure is fixed across solver iterations and only the values $v$ change. The efficiency of these five callbacks dominates the per-iteration cost of the solver, and a key goal of any AMS is to provide them as efficiently as possible.

The role of an algebraic modeling system (AMS) is to bridge the gap between the user's mathematical formulation and these solver callbacks. The AMS provides: (i) a user-facing syntax for expressing the model, (ii) an internal representation (typically an expression graph) for storing the model, (iii) automatic differentiation for evaluating derivatives, and (iv) sparsity detection for constructing the sparse Jacobian and Lagrangian Hessian structures. Established AMSs include AMPL \citep{fourerModelingLanguageMathematical1990}, JuMP \citep{lubinJuMP10Recent2023}, CasADi \citep{anderssonCasADiSoftwareFramework2019}, and Pyomo \citep{hartPyomoModelingSolving2011}.

An AMS operates over two distinct phases. \emph{Model creation} is the one-time work performed when the user constructs the model: parsing the user's expressions, building an internal representation, and analyzing sparsity. \emph{Derivative evaluation} is the per-callback work performed at every solver iteration to fill the outputs of the callbacks above. Solvers invoke each callback many times, so per-callback cost dominates the total solve time once the problem is large. ExaModels.jl is structured around this distinction: \Cref{sec:graph} describes the model-creation work, and \Cref{sec:ad} the derivative-evaluation work, with a single parameterized expression tree shared between them.

A common design pattern across these systems is that the model is \emph{flattened} into a monolithic expression graph during construction. That is, each scalar constraint and objective term is recorded as an individual node in a single large graph. While this approach is general and flexible, it discards the repetitive structure present in most engineering models. As a consequence, the AD engine must traverse the entire graph without knowledge of which subexpressions share the same algebraic pattern, precluding SIMD-parallel derivative evaluation.
...
\end{align}
\end{subequations}
where $x \in \mathbb{R}^{n}$ is the vector of decision variables with bounds $x^\flat, x^\sharp$; $f^{(\ell)}$, $g^{(m)}$, and $h^{(n)}$ are twice-differentiable scalar functions referred to as \emph{algebraic patterns}; and $p_i^{(\ell)}$, $q_j^{(m)}$, $s_k^{(n)}$ are problem data that parameterize each pattern instance. We use $[N] := \{1,\ldots,N\}$ to denote index sets.

The objective \cref{eq:simd:obj} is decomposed into $L$ groups of objective patterns, where pattern $f^{(\ell)}$ is evaluated over $I_\ell$ data points. The constraints \cref{eq:simd:con} consist of $M$ blocks: within block $m$, the ``base'' pattern $g^{(m)}$ defines $J_m$ constraints, and the ``augmentation'' patterns $h^{(n)}$ contribute additive terms to these constraints. This augmentation structure arises naturally in network models (e.g., summing arc flows into nodal balance equations) and is discussed further in \Cref{sec:data:core}.

The key observation is that many large-scale NLPs arising from engineering applications can be expressed in the form \cref{eq:simd} with a small number of distinct patterns, even when the problem involves millions of variables and constraints. The Goddard rocket \cref{eq:rocket} is a direct instance: the three dynamics constraints \cref{eq:rocket:h}--\cref{eq:rocket:m} are three patterns each evaluated over $T$ data points, and the four boundary conditions contribute four single-instance patterns, giving $L + M = 5$ patterns in total regardless of $T$. The ACOPF model requires only 15 patterns \citep{shinAcceleratingOptimalPower2024}---generation cost, power flows, voltage limits, power balance, and generator/line contributions---regardless of network size. More broadly, the \cref{eq:simd} captures the full range of COPS benchmark problems \citep{dolanBenchmarkingOptimizationSoftware2001}: optimal control problems (rocket steering, robot arm, hang glider), PDE-constrained optimization (minimal surface, elastic--plastic torsion, journal bearing), parameter estimation with collocation (catalyst mixing, gas--oil yield, marine population dynamics), and mesh/shape optimization (tetrahedral/triangular mesh smoothing, largest polygon, cam shape design). In all of these, the number of distinct patterns is small and fixed, while the problem size scales with the discretization resolution. This \emph{pattern--data separation} is the foundation of the SIMD abstraction.
...
In ExaModels.jl, each algebraic pattern is compiled into a \texttt{SIMDFunction} that provides a per-data-point callback $\tilde{f}(i, x, \theta)$, where $i$ is a data point, $x$ is the decision variable vector, and $\theta$ is the parameter vector. The global objective, for instance, is evaluated as
...
Concretely, each \texttt{SIMDFunction} bundles: (i) the parameterized expression tree $\tilde{f}$ (\Cref{sec:graph}), (ii) compressor maps for the Jacobian and Hessian that translate the AD traversal into sparse coordinate indices (\Cref{sec:ad:sparsity}), and (iii) integer offsets and strides that locate this pattern's contributions within the global output arrays. Given these offsets, each callback in the callbacks above reduces to a loop over patterns, where each pattern's \texttt{SIMDFunction} is evaluated across all its data points---an embarrassingly parallel operation that maps directly to a GPU kernel (\Cref{sec:gpu}). The \texttt{SIMDFunction} is constructed once during model compilation and is immutable thereafter; updating the decision variables $x$ or the parameters $\theta$ requires no recompilation.

\subsection{User-Facing Syntax}
\label{sec:data:syntax}

This section provides a concise overview of the modeling API. For a complete reference---including all function signatures, keyword arguments, and usage examples---we refer the reader to the online documentation \citep{shinExaModelsjl2025}.
...
\end{itemize}
To illustrate the end-to-end workflow, the following code builds and solves the Goddard rocket \cref{eq:rocket} from the introduction:
\begin{lstlisting}
...
@add_con(core, h[0] - h_0)            # boundary: initial altitude
@add_con(core, v[0] - v_0)            # boundary: initial velocity
@add_con(core, m[0] - m_0)            # boundary: initial mass
@add_con(core, m[nh] - m_f)           # boundary: final mass

model = ExaModel(core)                 # compile: ExaCore -> ExaModel
result = ipopt(model)                  # solve via Ipopt
\end{lstlisting}
The five patterns (\texttt{c1}--\texttt{c3} dynamics, boundary conditions, and the objective) directly correspond to \cref{eq:rocket}. The same model runs on a GPU by replacing the first line with \texttt{core = ExaCore(minimize = false, backend = CUDABackend())}; no other changes are needed.
...
\end{enumerate}
The resulting \texttt{ExaModel} is a subtype of \texttt{NLPModels.AbstractNLPModel\{T,VT\}}, where \texttt{T} is the floating-point type and \texttt{VT} is the array type (\texttt{Vector\{T\}} for CPU, \texttt{CuArray\{T\}} for CUDA, etc.). This means it can be passed directly to any solver in the JuliaSmoothOptimizers ecosystem---including Ipopt (via NLPModelsIpopt.jl) and MadNLP.jl \citep{shinMadNLPjl2025}---without any adapter code. The solver calls the five callbacks the callbacks above, and ExaModels.jl dispatches each callback to a loop over the stored \texttt{SIMDFunction}s.

\subsection{Internal Data Types}
\label{sec:data:internal}

This subsection describes the internal types that connect the data structures above to the expression trees of \Cref{sec:graph} and the AD machinery of \Cref{sec:ad}. We work top-down: from the wrapper types stored in \texttt{obj}/\texttt{cons} tuples, to the \texttt{SIMDFunction} that each wrapper bundles, to the expression-tree node types underneath.
...
\begin{itemize}
  \item \texttt{Objective\{F,I\}} --- wraps a \texttt{SIMDFunction} \texttt{f} and a collected data iterator \texttt{itr}. Corresponds to one objective pattern $\tilde{f}^{(\ell)}$ in \cref{eq:simd:obj}.
  \item \texttt{Constraint\{F,I,O,S,T\}} --- wraps a \texttt{SIMDFunction}, a data iterator, an offset into the global constraint vector, dimension information, and a tag. Corresponds to one base constraint pattern $\tilde{g}^{(m)}$ in \cref{eq:simd:con}.
  \item \texttt{ConstraintAugmentation\{F,I,D,T\}} --- wraps a \texttt{SIMDFunction} whose expression tree is a \texttt{Pair\{index\_expr, value\_expr\}}. The index expression determines \emph{which} constraint row to augment, and the value expression provides the additive term. Corresponds to the augmentation patterns $\tilde{h}^{(n)}$ in \cref{eq:simd:con}.
\end{itemize}
This three-type design keeps the number of expression trees minimal.

\paragraph{SIMDFunction.}
A \texttt{SIMDFunction\{F,C1,C2\}} bundles the compiled representation of a single algebraic pattern. It contains:
\begin{itemize}
  \item \texttt{f::F} --- the parameterized expression tree (an \texttt{AbstractNode}).
  \item \texttt{comp1::C1}, \texttt{comp2::C2} --- \texttt{Compressor} mappings for Jacobian and Lagrangian Hessian sparsity (\Cref{sec:ad:sparsity}).
...
\end{itemize}
The \texttt{SIMDFunction} is the central compilation artifact: it is constructed at model-construction time by probing the expression tree for sparsity and building the compressor maps, and it is the unit of work for both CPU loops and GPU kernels (\Cref{sec:gpu}).

\paragraph{AbstractNode and expression trees.}
The \texttt{f::F} field of each \texttt{SIMDFunction} is a tree of \texttt{AbstractNode} subtypes. A node is callable as \texttt{node(i, x, $\theta$)}: given a data point~$i$, a primal vector~$x$, and a parameter vector~$\theta$, it returns a scalar value. Interior nodes (\texttt{Node1}, \texttt{Node2}) store a function and references to their children; leaf nodes (\texttt{Var}, \texttt{DataIndexed}, \texttt{Constant}, \texttt{DataSource}) provide access to variables, data, compile-time constants, and the data-point sentinel, respectively. The full node hierarchy and the multiple-dispatch machinery that constructs the tree are deferred to \Cref{sec:graph}. To see why \texttt{ConstraintAugmentation} is essential, consider the active power balance constraint in ACOPF:
\begin{equation*}
...
\begin{lstlisting}
@add_con(c, c_p, -b.pd for b in bus_data)             # base: one row per bus
@add_con!(c, c_p, g.bus => -pg[g.i] for g in gen_data) # generator injection
@add_con!(c, c_p, l.from => pf[l.i] for l in line_data) # line flow (from end)
@add_con!(c, c_p, l.to   => pt[l.i] for l in line_data) # line flow (to end)
\end{lstlisting}
...
During model creation, ExaModels.jl captures each user-supplied generator body---objective or constraint---as a single \emph{parameterized expression tree}: a Julia data structure that represents one algebraic pattern from \cref{eq:simd} and is evaluated across every data point in the pattern without per-instance unrolling. Unlike conventional expression graphs that store one tree per scalar expression, a single parameterized expression tree represents an entire family of expressions---one for each data point---which is what enables the SIMD evaluation and differentiation of \Cref{sec:modeling:simd}. This section describes the tree representation, the source sentinels that drive its construction, and the registered-function machinery that builds it from natural mathematical syntax.

\subsection{Source Sentinels}
\label{sec:graph:sources}

Consider a constraint pattern such as
\begin{lstlisting}
@add_con(c, x[i+1]^2 - x[i] for i = 1:N-1)
\end{lstlisting}
The macro lifts the generator body into a Julia function $f(i, x, \theta)$ taking a data point $i$, the primal vector $x$, and the parameter vector $\theta$. Because $f$ dispatches on the types of its arguments, ExaModels.jl can call it with different \emph{source sentinels} in place of the natural numeric arguments and obtain different outputs from a single function definition: a symbolic expression tree, a numeric value, an AD-augmented value, or a sparsity record.

At tree-construction time, $f$ is called with \texttt{i = DataSource()}, \texttt{x = VarSource()}, and \texttt{$\theta$ = ParameterSource()}. The sentinels' \texttt{getindex} and \texttt{getproperty} methods dispatch to constructors that return the symbolic leaves \texttt{DataIndexed}, \texttt{Var}, and \texttt{ParameterNode} (\Cref{sec:graph:nodes}); the registered arithmetic operators (\Cref{sec:graph:register}) compose them into interior \texttt{Node1}/\texttt{Node2} nodes. The result is a parameterized expression tree whose evaluation rule is the same function $f$, now called with numeric arguments at run time.

The remaining source types---\texttt{AdjointNodeSource}, \texttt{SecondAdjointNodeSource}, and the sparsity-probe sentinels---are introduced in \Cref{sec:ad}, where they drive the AD reverse passes.
...
\begin{itemize}
  \item \textbf{Leaf nodes} supply inputs:
    ...
    \texttt{Constant\{T\}} embeds a compile-time constant as a type parameter (\Cref{sec:graph:constants}).
...
\paragraph{Example.}
To illustrate, consider the extended Rosenbrock objective from the Luk\v{s}an--Vlc\v{e}k benchmark \citep{lukVsanSparsePartiallySeparable}:
...
During derivative evaluation, ExaModels.jl reuses the parameterized expression tree of \Cref{sec:graph} to compute the gradient, Jacobian, and Lagrangian Hessian required by the solver callbacks the callbacks above. The same Julia function $f(i, x, \theta)$ that built the tree at model creation also produces derivatives when $x$ is replaced by a derivative-carrying source sentinel: an \texttt{AdjointNodeSource} for first order, or a \texttt{SecondAdjointNodeSource} for second order. Each \texttt{Var} leaf then returns a derivative-augmented value, and the registered overloads of \Cref{sec:graph:register} propagate derivatives through the same expression structure. Reverse-mode AD in ExaModels.jl is not a separate code path; it is a re-evaluation of the same tree with a different source type, followed by a reverse traversal that accumulates adjoints into a sparse coordinate output.

Three broad approaches to derivative computation exist. \emph{Symbolic differentiation} applies the chain rule at the level of mathematical expressions, producing closed-form derivative expressions; this can lead to expression swell for complex functions and is difficult to apply to general programs. \emph{Finite differences} approximate derivatives via function evaluations at perturbed points (e.g., $(f(x+he_j) - f(x))/h$); this is straightforward to implement but introduces truncation error, is expensive for large $n$ (requiring $O(n)$ evaluations for a gradient), and is numerically unreliable for second-order derivatives. \emph{Automatic differentiation} (AD) computes exact derivatives of a function specified as a computer program by systematically applying the chain rule to each elementary operation, achieving machine-precision accuracy at a cost proportional to the function evaluation itself.

Within AD, two fundamental modes exist: \emph{forward mode}, which propagates directional derivatives from inputs to outputs, and \emph{reverse mode}, which propagates adjoint (sensitivity) information from outputs to inputs. For a function $f:\mathbb{R}^n \to \mathbb{R}$, forward mode requires $n$ passes to compute the full gradient, while reverse mode computes it in a single pass---making reverse mode the natural choice for the NLP callbacks the callbacks above, where $n$ is typically large.
...
ExaModels.jl takes a hybrid approach that combines advantages of both. Operator overloading via Julia's multiple dispatch is used to construct the parameterized expression tree at model-construction time (\Cref{sec:graph:construction}): arithmetic on \texttt{AbstractNode} operands builds symbolic \texttt{Node1}/\texttt{Node2} nodes rather than computing numeric values. Once the expression tree is built, ExaModels.jl applies its own reverse-mode AD algorithm directly on this symbolic graph---a form of expression-graph-based AD analogous to the approach used by CasADi \citep{anderssonCasADiSoftwareFramework2019} and AMPL \citep{fourerModelingLanguageMathematical1990}. Because the expression tree type is fully concrete and known at compile time (\Cref{sec:graph:type}), the Julia compiler aggressively inlines the AD traversal, producing specialized machine code for each pattern with no interpretive overhead. The result is a system where the user writes natural mathematical expressions, dispatch builds the graph, and the compiler produces code whose performance is competitive with hand-written source transformations.

This section describes the reverse-mode AD algorithm used by ExaModels.jl, including first-order (gradient/Jacobian) and second-order (Lagrangian Hessian) derivative computations, and the coloring-free sparsity detection that exploits the SIMD structure.

\subsection{First-Order Derivatives}
\label{sec:ad:first}

Given a parameterized expression tree $\tilde{f}$ (\Cref{sec:graph}), ExaModels.jl computes the gradient $\nabla_x \tilde{f}$ via a two-phase procedure: a \emph{forward pass} that builds an augmented tree, and a \emph{reverse pass} that propagates adjoints from root to leaves.

\paragraph{Forward pass.}
The forward pass evaluates the expression tree with the primal vector $x$ replaced by an \texttt{AdjointNodeSource(x)}. Each registered function (\Cref{sec:graph:register}) has an adjoint overload that, instead of returning a scalar, returns an \texttt{AdjointNode} storing both the primal value and the local partial derivatives:
\begin{itemize}
...
\Cref{fig:ad} illustrates both passes on the Rosenbrock pattern from \cref{eq:rosenbrock}, using the same expression tree as \Cref{fig:tree}. For readability we write $a = x_{i-1}$ and $b = x_i$, and let $s = a^2 - b$, $t = a - 1$.
...
\paragraph{Remark.}
The forward--reverse AD procedure described above is standard in the AD literature. What distinguishes ExaModels.jl is that these passes are applied to the \emph{parameterized} expression tree from \Cref{sec:graph}: the same compiled AD code is executed across all data points of a pattern in an embarrassingly parallel fashion. For a pattern with $I$ data points, the forward and reverse passes are independent across $i \in [I]$---each invocation reads different variable indices (via \texttt{DataIndexed}) and writes to different sparse output positions (via the \texttt{Compressor} offsets). This data-parallel structure maps directly to the SIMD abstraction of \Cref{sec:modeling:simd} and is the basis for GPU acceleration (\Cref{sec:gpu}).
...
The heavy lifting for GPU support has already been done in \Cref{sec:modeling,sec:graph}: the SIMD abstraction expresses each pattern as a single function evaluated over a data iterator, and the parameterized expression tree is type-stable end-to-end so that the Julia compiler specializes a dedicated evaluator per pattern. What remains is to apply the AD passes of \Cref{sec:ad} as GPU kernels. The mapping is direct: each algebraic pattern becomes a GPU kernel, and each data point becomes a thread. Because all threads within a pattern execute the same compiled expression tree on different data indices, there is no thread divergence---every thread follows the same control flow, which is ideal for GPU hardware. This section describes how ExaModels.jl implements this mapping, using KernelAbstractions.jl~\citep{churavyKernelAbstractionsjl2025} for backend-agnostic GPU kernels, and discusses the key implementation choices that affect performance on both CPU and GPU.
...
KernelAbstractions.jl~\citep{churavyKernelAbstractionsjl2025} provides the backend abstraction layer. All kernels are written once using the \texttt{@kernel} macro and run on any supported backend: CUDA.jl (NVIDIA), AMDGPU.jl (AMD), oneAPI.jl (Intel), Metal.jl (Apple), and the \texttt{CPU()} backend for multithreaded execution on the host. The user selects the backend at model-construction time via \texttt{ExaCore(; backend = CUDABackend())}, and no model code changes are needed.
...
\end{lstlisting}
This API is natural and ergonomic, but the model's concrete type does not change across these calls---all structure is hidden in runtime data structures. As a result, the derivative evaluation code necessarily involves dynamic dispatch over the stored expression types, precluding the kind of compile-time specialization described in \Cref{sec:graph:type,sec:gpu:cpugpu}.
...
\end{lstlisting}
This extends to user-defined numeric types. For example, using DoubleFloats.jl for quadruple precision requires no code changes beyond specifying the type at \texttt{ExaCore} construction. The \texttt{Constant\{T\}} type parameter (\Cref{sec:graph:constants}) ensures that compile-time constants are converted to the appropriate precision via \texttt{replace\_T}, so no mixed-precision artifacts arise.
...
\end{itemize}
Setting $S = 1$ and $B = 1$ recovers \cref{eq:simd}. Setting $B = 1$ gives a standard two-stage stochastic program. Setting $S = 1$ with no shared variables gives a batch model. All three can be active simultaneously.
...
The two-stage core can be combined with batching (\Cref{sec:extensions:batch}): \texttt{TwoStageExaCore(nscen; nbatch = Val(B))} creates a batched two-stage core. The following example illustrates all three orthogonal extensions simultaneously:
\begin{lstlisting}
B, S = 4, 10                              # 4 instances, 10 scenarios each
core = TwoStageExaCore(S; nbatch = Val(B))
@add_var(core, d, n_d)                    # design (shared across scenarios)
@add_var(core, EachScenario(), v, n_v)    # recourse (per scenario)
@add_par(core, EachScenario(), θ_vals)                 # scenario parameters
@add_obj(core, f0(d, i) for i in data0)                # first-stage obj
@add_obj(core, EachScenario(), fs(d, v, θ, i) for i in data_s) # 2nd-stage obj
@add_con(core, EachScenario(), gs(d, v, θ, i) for i in data_s) # 2nd-stage cons
m = ExaModel(core)
\end{lstlisting}
...
\texttt{BatchExaCore(nbatch)} creates a core for \texttt{nbatch} independent instances. All declarations are implicitly replicated across instances; internally, the storage switches from vectors to matrices of size $n \times B$. The batch structure ($B > 1$) solves $B$ fully independent NLP instances that share the same compiled expression trees but differ in parameter values. The NLPModels.jl callbacks evaluate all $B$ instances in a single kernel launch on the GPU (or a single vectorized loop on the CPU), with each data-point--instance pair mapped to a separate thread. This provides two levels of parallelism: across data points within a pattern, and across instances.
...
\subsection{External Oracles}
\label{sec:extensions:oracles}

Some model components are not naturally expressed as a parameterized expression tree: neural-network layers, complex simulation residuals, PDE solvers, or custom GPU kernels. For these cases, ExaModels.jl provides an \emph{external oracle} mechanism (currently under development) that lets users register callbacks for function evaluation, Jacobian, and Hessian computation. The oracle bypasses ExaModels' built-in AD; the user supplies derivatives directly, and ExaModels integrates the oracle's outputs into the global COO arrays.

External oracles come with several limitations that follow from their design as escape hatches:
\begin{itemize}
  \item \textbf{No SIMD parallelism.} An oracle call is a black box; ExaModels cannot factor out shared computation across instances or parallelize evaluation over a data dimension.
  \item \textbf{Manual derivatives.} The user is responsible for the correctness, sparsity pattern, and storage layout of the Jacobian and Hessian contributions; mistakes are silent.
  \item \textbf{Reduced compile-time specialization.} Dynamic dispatch through the oracle callback breaks type stability across that boundary, and the compiler cannot inline through it.
  \item \textbf{GPU compatibility is the user's responsibility.} If the oracle is not implemented as a GPU-native kernel, callbacks fall back to host-side execution, breaking the GPU-resident pipeline.
  \item \textbf{Composition.} Oracle outputs must be inserted into the global Jacobian and Hessian COO arrays at the correct offsets; ExaModels provides scaffolding, but the user must align the index conventions.
\end{itemize}
External oracles are an escape hatch, not a primary modeling primitive. For problems that fit the SIMD abstraction, the templatized form is preferable.
...
In the summary table (\Cref{tab:sgm}), we classify instances into three size categories based on $\text{nnz} = \max(\text{nnzj}, \text{nnzh})$: \emph{Small} ($\text{nnz} < 10^3$), \emph{Medium} ($10^3 \leq \text{nnz} < 10^5$), and \emph{Large} ($10^5 \leq \text{nnz}$). This classification captures the regime where data-parallel AD provides the most benefit (large problems with many nonzeros) versus the regime where fixed overheads dominate (small problems).

\paragraph{Hardware and software.}
\Cref{tab:hardware} summarizes the hardware used in the experiments. The AD comparison (\Cref{sec:numerics:ad}) runs single-threaded on the CPU of Machine~A. The GPU experiments (\Cref{sec:numerics:gpu}) compare this CPU baseline against two GPU backends on the same machines. All experiments use Julia~1.12.6, ExaModels.jl~v0.10.0, and Float64 arithmetic.

\begin{table}[t]
\centering
\caption{Hardware configurations used in the benchmark experiments.}
\label{tab:hardware}
\begin{tabular}{lllr}
  \toprule
  Machine & CPU & GPU & RAM \\
  \midrule
  A & Intel Xeon Platinum 8160 (2.10\,GHz, 96 cores) & NVIDIA Quadro GV100 (32\,GB) & 377\,GB \\
  B & Intel Xeon Gold 6130 (2.10\,GHz, 64 cores) & AMD Radeon VII (16\,GB) & 251\,GB \\
  \bottomrule
\end{tabular}
\end{table}
...
We compare the time to evaluate each NLPModels.jl callback---\texttt{obj}, \texttt{cons!}, \texttt{grad!}, \texttt{jac\_coord!}, and \texttt{hess\_coord!}---as well as the model creation time, across ExaModels.jl, JuMP~\citep{lubinJuMP10Recent2023}, and AMPL~\citep{fourerModelingLanguageMathematical1990} (via AmplNLReader.jl). All timings report the minimum wall time over multiple trials, which is robust to system noise.
...
\begin{table}[t]
  ...
  \label{tab:sgm}
  \input{benchmark/results/tables/sgm_summary} 
\end{table} 
...
Large-scale nonlinear programs almost always exhibit highly repetitive structure, yet existing algebraic modeling systems often neglect it. We have presented ExaModels.jl, an algebraic modeling system for nonlinear programming implemented in Julia that preserves and exploits this structure. Two ideas underpin the system:
\begin{enumerate}
  \item A \emph{single-instruction, multiple-data abstraction} that represents a nonlinear program as a small number of algebraic patterns, each evaluated over many data points, realized through a parameterized expression tree fully encoded in the Julia type system. Because problem structure is recognized at compile time, function and derivative evaluation reduces to embarrassingly parallel loops over data points, making the system natively compatible with GPUs.
  \item A \emph{coloring-free sparse automatic differentiation algorithm} that exploits regular per-pattern sparsity to compute Jacobians and Hessians in partially compressed sparse form without graph coloring.
\end{enumerate}
Compile-time specialization delivers speedups in sparse derivative evaluation over existing algebraic modeling systems on CPU, and data-parallel execution yields up to two orders of magnitude further acceleration on GPUs. The implementation supports arbitrary numeric precision, cross-platform GPU portability across NVIDIA, AMD, Intel, and Apple devices via KernelAbstractions.jl, ahead-of-time compilation to standalone binaries independent of the Julia runtime, and extensions to parametric problems, user-defined external oracles, two-stage stochastic programs, and batch optimization. ExaModels.jl connects to a broad solver ecosystem through the NLPModels.jl callback interface, including Ipopt, MadNLP.jl, and Uno.

\paragraph{Limitations and future work.}
The current system has several limitations that suggest directions for future work. First, the requirement that all instances of a pattern share the same expression tree structure means that problems with data-dependent branching (e.g., complementarity constraints with different active sets) cannot be expressed as a single pattern. More broadly, certain model components---such as neural network layers or complex simulation residuals---are not naturally expressed as parameterized expression trees. For such cases, ExaModels.jl provides a \emph{nonlinear oracle} mechanism (currently under development) that allows users to register external callbacks for function evaluation, Jacobian, and Hessian computation. The oracle bypasses ExaModels' built-in AD and integrates external derivative information directly into the NLPModels.jl callbacks, enabling hybrid models where some components are expressed as SIMD patterns and others are handled by external tools (e.g., Flux.jl for neural networks, or custom GPU kernels for PDE residuals). Second, AOT compilation currently supports CPU backends only; extending it to GPU backends requires advances in Julia's GPU compilation pipeline. Third, the parameterized expression tree approach requires that the user express the problem in the SIMD form \cref{eq:simd}, which may require restructuring compared to the monolithic formulation natural in JuMP or AMPL. Finally, while ExaModels.jl evaluates derivatives efficiently, the overall solve time is dominated by the linear algebra within the interior-point solver; realizing the full benefit of GPU acceleration therefore requires a GPU-native solver such as MadNLP.jl~\citep{shinMadNLPjl2025} with condensed-space methods~\citep{pacaudCondensedspaceMethodsNonlinear2024}.
...
\label{app:tables:lv}
{\footnotesize
\input{benchmark/results/tables/appendix_LV}
}

\subsection{COPS Problems}
\label{app:tables:cops}
{\footnotesize
\input{benchmark/results/tables/appendix_COPS}
}

\subsection{PGLIB-OPF (Polar Formulation)}
\label{app:tables:opf-polar}
{\footnotesize
\input{benchmark/results/tables/appendix_OPF-polar}
}

\subsection{PGLIB-OPF (Rectangular Formulation)}
\label{app:tables:opf-rect}
{\footnotesize
\input{benchmark/results/tables/appendix_OPF-rect}
}
