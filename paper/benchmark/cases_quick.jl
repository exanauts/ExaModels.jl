# ============================================================================
# Quick benchmark cases (reduced set for laptop runs)
# ============================================================================
...
const COPS_CASES = [
    (name = "bearing",     model = COPSBenchmark.bearing_model,     sizes = [(50,50), (100,100)]),
    (name = "camshape",    model = COPSBenchmark.camshape_model,    sizes = [3200, 6400]),
    (name = "catmix",      model = COPSBenchmark.catmix_model,      sizes = [1600, 3200]),
    (name = "chain",       model = COPSBenchmark.chain_model,       sizes = [3200, 6400]),
    (name = "channel",     model = COPSBenchmark.channel_model,     sizes = [3200, 6400]),
    (name = "elec",        model = COPSBenchmark.elec_model,        sizes = [200, 400]),
    (name = "gasoil",      model = COPSBenchmark.gasoil_model,      sizes = [1600, 3200]),
    (name = "glider",      model = COPSBenchmark.glider_model,      sizes = [1600, 3200]),
    (name = "marine",      model = COPSBenchmark.marine_model,      sizes = [800, 1600]),
    (name = "methanol",    model = COPSBenchmark.methanol_model,    sizes = [1600, 3200]),
    (name = "minsurf",     model = COPSBenchmark.minsurf_model,     sizes = [(50,50), (100,100)]),
    (name = "pinene",      model = COPSBenchmark.pinene_model,      sizes = [1600, 3200]),
    (name = "polygon",     model = COPSBenchmark.polygon_model,     sizes = [1600, 3200]),
    (name = "robot",       model = COPSBenchmark.robot_model,       sizes = [800, 1600]),
    (name = "rocket",      model = COPSBenchmark.rocket_model,      sizes = [6400, 12800]),
    (name = "steering",    model = COPSBenchmark.steering_model,    sizes = [6400, 12800]),
    (name = "torsion",     model = COPSBenchmark.torsion_model,     sizes = [(50,50), (100,100)]),
    (name = "dirichlet",   model = COPSBenchmark.dirichlet_model,   sizes = [20, 40]),
]
