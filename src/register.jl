macro register_univariate(f,df,ddf)
    return esc(
        quote
            @inline $f(n::N) where {N <: ExaModels.AbstractNode} =
                ExaModels.Node1($f,n)
            ...
            @inbounds @inline (n::ExaModels.Node1{typeof($f),I})(i,x) where I = $f(n.inner(i,x))
        end
    )
end
    ...
    return esc(
        quote
            @inline function $f(d1::D1, d2::D2) where {
                D1 <: ExaModels.AbstractNode,
                D2 <: ExaModels.AbstractNode
                }
                ExaModels.Node2(
                    $f,d1,d2
                )
            ...
            @inbounds @inline (n::ExaModels.Node2{typeof($f),I1,I2})(i,x) where {I1, I2} = $f(n.inner1(i,x), n.inner2(i,x))
            @inbounds @inline (n::ExaModels.Node2{typeof($f),I1,I2})(i,x) where {I1 <: Real, I2} = $f(n.inner1, n.inner2(i,x))
            @inbounds @inline (n::ExaModels.Node2{typeof($f),I1,I2})(i,x) where {I1, I2 <: Real} = $f(n.inner1(i,x), n.inner2)
        end
    )
end
