...
function add_var(
    ...
    nscen = c.tag.nscen
    len = total(ns)
    append!(c.backend, c.tag.var_scen, _scen_full_tag(nscen, len), len)
    return _add_var(
        c, SecondStageTag(), name, start, lvar, uvar, ns...
    )
end
...
function add_con(
    ...
    append!(c.backend, c.tag.con_scen, 0, length(gen))
    return _add_con(
...
function add_con(
    ...
    return _add_con(
        c, gen, start, lcon, ucon, name, SecondStageTag()
    )
end
