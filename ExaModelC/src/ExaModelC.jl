...
- `build(c, args)` — the model, written against the lazy core exactly as against
...
the convention consumed by CNLPModels.jl. The core
...
const CORE = build(ExaModels.ExaCore(concrete = Val(true)), ExaModels.ArgTracer())
""",
...
Base.@ccallable function $(p("new_from_data"))(bid::Cint)::Cint
    ...
    data = (; $(join(asm, ",\n       ")))
    try
        push!(MODELS, ExaModels.ExaModel(CORE, data))
...
Base.@ccallable function $(prefix)_new(n::Cint)::Cint
    try
        push!(MODELS, ExaModels.ExaModel(CORE, make_data(Int(n))))
        return Cint(length(MODELS))
    catch
        return Cint(0)
    end
...
$setup
const ModelT = typeof(ExaModels.ExaModel(CORE, $template_call))
const MODELS = ModelT[]
...
_assert_value(e, x::ExaModels.ArgRange) =
    (_assert_value(e, x.lo); _assert_value(e, x.step); _assert_value(e, x.hi))
_assert_value(e, x::Function) =
    _serializable_root(typeof(x)) || throw(
        ArgumentError(
            "the core carries a function defined outside ExaModels/Base " *
...
# A lazy core serializes when nothing in it carries a foreign closure:
# expression trees are closure-free by construction (traced at add_*), so
# what can smuggle one in is a value slot — a generator start spec inside a
# pending array chain, or a deferred comprehension body.
_assert_value(e, p::ExaModels.PendingVec) =
    (_assert_value(e, p.base); _assert_value(e, p.spec); _assert_value(e, p.len))
function _assert_serializable(core::ExaModels.ExaCore)
    for name in (:x0, :θ, :lvar, :uvar, :y0, :lcon, :ucon)
        _assert_value(name, getproperty(core, name))
    end
    for e in (core.var..., core.par..., core.obj..., core.cons...)
...
function compile_library(
    ...
    Serialization.serialize(joinpath(appdir, "src", "core.jls"), core)
    Serialization.serialize(joinpath(appdir, "src", "template.jls"), template)

    fp = _fingerprint(
        read(joinpath(appdir, "src", "core.jls")),
