...
function runtests()
    @testset "TwoStageExaCore" begin
        ...
        @testset "Variable and constraint scenario tags" begin
            ...
            core = TwoStageExaCore()
            @var(core, d; start = 0.0, scenario = 0)
            @var(core, v, ns, nv; start = 1.0, scenario = [s for s in 1:ns, j in 1:nv])
            ...
            @con(core, v[s, 1] - v[s, 2] for s in 1:ns; scenario = 1:ns)
        ...
        @testset "Build and solve simple two-stage model" begin
            ...
            core = TwoStageExaCore()
            @var(core, d; start = 1.0, lvar = 0.0, uvar = Inf, scenario = 0)
            @var(core, v, ns, nv; start = 1.0, lvar = 0.0, uvar = Inf, scenario = [s for s in 1:ns, j in 1:nv])
        ...
        @testset "Design-only model (no recourse)" begin
            core = TwoStageExaCore()
            @var(core, d, 3; start = 1.0, scenario = 0)
            @obj(core, (d[i] - 2)^2 for i in 1:3)
