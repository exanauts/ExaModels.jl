...
function runtests()
    @testset "TwoStageExaCore" begin
        ...
        @testset "Variable and constraint scenario tags" begin
            ...
            core = TwoStageExaCore()
            @add_variable(core, d; start = 0.0, scenario = 0)
            @add_variable(core, v, ns, nv; start = 1.0, scenario = [s for s in 1:ns, j in 1:nv])
            ...
            @add_constraint(core, v[s, 1] - v[s, 2] for s in 1:ns; scenario = 1:ns)
        ...
        @testset "Build and solve simple two-stage model" begin
            ...
            core = TwoStageExaCore()
            @add_variable(core, d; start = 1.0, lvar = 0.0, uvar = Inf, scenario = 0)
            @add_variable(core, v, ns, nv; start = 1.0, lvar = 0.0, uvar = Inf, scenario = [s for s in 1:ns, j in 1:nv])

            @add_constraint(core, v[s, 1] - v[s, 2]^2 for s in 1:ns; lcon = 0.0, scenario = 1:ns)

            @add_objective(core, d^2)
            @add_objective(core, weight * (v[s, i] - d)^2 for s in 1:ns, i in 1:nv)
        ...
        @testset "Design-only model (no recourse)" begin
            core = TwoStageExaCore()
            @add_variable(core, d, 3; start = 1.0, scenario = 0)
            @add_objective(core, (d[i] - 2)^2 for i in 1:3)
