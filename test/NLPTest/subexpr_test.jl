...
function test_subexpr_basic(backend)
    ...
    x1 = variable(c1, dimx; start = 1.0)
    # Objective: sum of (x[i]^2 + x[i+1]^2)^2
    objective(c1, (x1[i]^2 + x1[i + 1]^2)^2 for i in 1:(dimx-1))
    ...
    x2 = variable(c2, dimx; start = 1.0)
    # Create subexpression for x[i]^2
    s = subexpr(c2, x2[i]^2 for i in 1:dimx)
    # Objective using subexpression
    objective(c2, (s[i] + s[i + 1])^2 for i in 1:(dimx-1))
    # Constraint using subexpression
    constraint(c2, s[i] - 1 for i in 1:dimx; lcon = 0.0)
    m2 = ExaModel(c2)

    # The model with subexpressions has more variables and constraints
    @test m2.meta.nvar == m1.meta.nvar + dimx
